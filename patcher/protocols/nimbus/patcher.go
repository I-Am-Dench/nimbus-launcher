...
func (p Patcher) FetchManifest(ctx context.Context, name string, manifestFile, hotfixFile *manifest.Manifest, atRoot ...bool) (*manifest.Manifest, error) {
	...
	if hotfixFile != nil {
		manifestFile.AddEntries(hotfixFile.Entries()...)
	}
...
func (p Patcher) collectEntries(ctx context.Context, name string, index, hotfix *manifest.Manifest, archive ...*archive.Archive) ([]manifest.Entry, error) {
	manifestFile, err := p.FetchManifest(ctx, name, index, hotfix, true)
	if err != nil {
		return nil, err
	}
...
func (p Patcher) GetVersion(ctx context.Context, packed bool) (*archive.Archive, error) {
	...
	p.index, err = p.FetchManifest(ctx, IndexFile, version, nil)
	if err != nil {
		return nil, fmt.Errorf("patcher: %w", err)
	}
