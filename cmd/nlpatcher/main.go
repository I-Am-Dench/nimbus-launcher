...
func main() {
	...
	if masterIndex.Config.Type != patcherId {
		log.Fatalf("expected patcher %s but Master Index returned %s", patcherId, masterIndex.Config.Type)
	}
	...
	patcher, err := env.NewPatcher(ctx, patcher.Options{
		Resources: resources,
	...
	if err := undoer.Undo(patch.Archive()); err != nil {
		log.Println(err)
	}
